  ...
  for (let i = 0; i < count; i++) {
    ...
    feathers.push(
      <g key={i} transform={`translate(${originX} ${originY}) rotate(${angle})`}>
        <polygon points={`0,0 ${len},${-w} ${len},${w}`} fill={i % 2 === 0 ? colorA : colorB} stroke={colorB} strokeWidth="0.6" />
      </g>
    );
  }
  return <>{feathers}</>;
...
export default function BirdSVG({ speciesKey, colorKey, stage, size = 76 }) {
  ...
  if (stage === "egg") {
    return (
      <svg viewBox="0 0 100 100" width={size} height={size}>
        <ellipse cx="50" cy="58" rx="26" ry="34" fill={col.accent} stroke={col.wing} strokeWidth="2.5" />
  ...
  return (
    <svg viewBox="0 0 100 100" width={size} height={size}>
      ...
      <ellipse cx={cx} cy={cy} rx={bodyRx} ry={bodyRy} fill={col.base} />
      <ellipse cx={cx + 3 * s} cy={cy + 1} rx={bodyRx * 0.62} ry={bodyRy * 0.5} fill={col.wing} transform={`rotate(-8 ${cx} ${cy})`} />
      ...
      <circle cx={headCx} cy={headCy} r={headR} fill={col.base} />
      <polygon points={`${headCx - headR - 1},${headCy + 1} ${headCx - headR - 9 * s},${headCy - 2 * s} ${headCx - headR - 9 * s},${headCy + 4 * s}`} fill="#E0A24A" />
